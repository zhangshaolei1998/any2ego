...
Egocentric video is the format in which embodied agents see the world, with
the hands inside the field of view, a viewpoint that moves with the head, and
heavy self-occlusion of the manipulated object~\citep{grauman2024egoexo4d}, and
manipulation policies, wearable assistants, and action anticipation all consume
data in this form. However, it is also the format that is hardest to obtain at
scale, because every clip needs a head-mounted camera on a real wearer, whereas
third-person recordings of the same activities exist in quantity. Egocentric
viewpoint translation, which synthesizes the first-person video of a manipulation
task from a third-person observation of it, therefore turns a plentiful resource
into a scarce one. However, the value of such data rests entirely on whether the
generated video preserves the interaction that the demonstration shows.
...
intermediate quantity to supervise or inspect when the output is wrong. A person works
in two steps instead. Given a demonstration, a person first resolves where the
interaction is, which hand acts on which object and how they contact, and then
resolves what it looks like from the first person, where the objects fall in the
field of view and how the scene appears. A direct mapping collapses both steps
into one.
...
across views to supply the layout of hands and manipulated objects, and
EgoX~\citep{kang2025egox} builds a geometric prior from depth, point clouds, and
camera poses. However, these answer only the first of the two questions above,
since a mask or a depth map specifies neither object appearance, nor the meaning
of a contact, nor how the scene reads from the wearer's eyes. More recently, a line of work has explored general video
backbones such as Stable Video Diffusion~\citep{blattmann2023svd},
ConsistI2V~\citep{ren2024consisti2v}, Wan~\citep{wan2025}, and
VACE~\citep{vace2025}, which pair generative capacity with generic conditioning
interfaces. Although they accept visual and textual controls, they perform no
cross-view conversion of their own and offer no mechanism deciding where an
external semantic condition takes effect. In essence, every explicit intermediate representation proposed so far is
...
hand, and object. The predictor is trained on 108{,}338 annotated clips and
exports masks in the generator's input format, which makes the spatial anchor
of Eq.~\ref{eq:pipeline} a predicted quantity in every experiment.
...
so naive imitation of a teacher caption spends most of its gradient on content the
generator already sees. Two training stages address this.

\textbf{Difference-weighted Caption Distillation.}\quad To concentrate supervision
on what the existing conditions fail to carry, we compare each teacher sentence
against what the converter itself would say from the deployable inputs alone.
Teacher captions $c^\star$ come offline from a 27B vision-language model that
reads the paired egocentric footage, and a baseline caption $b$ is generated from
$u$ alone. Aligning each teacher sentence with $b$ separates new from redundant
content, and we set the token weight $w_j$ to $2.5$, $0.3$, and $1.0$ for new,
redundant, and partially overlapping content or field labels. For a target of length $L$, the
...
rendered video instead of on text similarity. For each input the rollout policy
samples $G=8$ candidates, a frozen generator renders a preview
$\widehat{Y}_i$ for candidate $c_i$, and the preview is cropped to the union
bounding box of the HOI masks so the reward is measured where the interaction
is. Let $\mathcal{S}_i$ be the sentences of $c_i$ that are new with respect to
$b$, let $C_M$ be the crop operator, and let $e_I$ and $e_T$ be
unit-normalized CLIP~\citep{radford2021clip} image and text embeddings. With
$F$ preview frames, the reward is
\begin{equation}
...
video token by the dense spatial features of the control branch. (b)
Difference-weighted distillation concentrates the gradient on caption content
that the deployable inputs do not already express, and generation-grounded
reinforcement scores each candidate by how well a rendered preview agrees with
its newly added sentences inside the interaction region.}
\vspace{-4mm}
\end{figure}
...
\label{eq:route}
\end{equation}
where $m_i$ is the pre-block HOI feature of the control branch aligned with
video token $i$, $\operatorname{sg}$ stops gradients so the spatial branch is
unaffected, and $\sigma$ is the sigmoid. Setting $s_i^{(\ell)}\!=\!1$ recovers a
global gate that scales the branch by one scalar, and $g^{(\ell)}\!=\!0$ removes
...
global gate, and spatial routing differ in exactly one design decision.
...
Section~\ref{sec:data}. Generation and evaluation use 13-frame clips at
$480\times480$ with 50 denoising steps and a fixed seed, under one shared
frame-loading pipeline. All training and inference run on NVIDIA A800 GPUs.
...
highest composite score of $4.043$, outperforming the strongest off-the-shelf
model by $0.915$ although that model is nearly thirty times larger, and exceeding
its own untrained backbone by $1.214$. More importantly, the two stages contribute differently, as
distillation lifts label-mean from $3.164$ to $4.160$ by teaching the six-field
first-person form, while generation-grounded reinforcement adds $0.121$
label-mean and $0.354$ composite by favoring content the generator can render.
...
\begin{table}[t]
\centering\small
\caption{The textual anchor improves every metric over EgoExo-Gen, which we
reproduce on the same backbone, while continuous vision-language features fail to
improve its FVD. Frame-level metrics saturate on short horizons, since the
freeze-frame calibration outscores all six open-source generators on SSIM and
PSNR while ranking near the bottom on FVD. Sees exo indicates whether a model observes the
third-person video. Best values are in \textbf{bold}.}
\label{tab:main}
...
$\;\;+$ VLM features, global gate & yes & 0.618 & 21.22 & 0.184 & 44.5 \\
\midrule
\multicolumn{6}{c}{\textbf{\textit{Any2Ego}}} \\
\midrule
$\;\;+$ textual anchor, global gate & yes & 0.636 & 21.74 & 0.163 & 37.8 \\
$\;\;+$ textual anchor, spatial routing & yes & \textbf{0.660} & \textbf{22.28} & \textbf{0.144} & \textbf{33.3} \\
\bottomrule
\end{tabular}
\vspace{-4mm}
\end{table}
...
same backbone, the Any2Ego rows isolate what the textual anchor adds to it. As
shown in Table~\ref{tab:main}, EgoExo-Gen already reaches $42.6$ FVD against
$121.6$ for the strongest image-to-video baseline, and the textual anchor lowers
FVD to $37.8$ under a global gate and to $33.3$ under spatial routing, reducing
LPIPS by $25.8\%$. More importantly, the freeze-frame row shows why the frame-level columns must be
read with care, as a video with no generated motion attains $0.601$ SSIM at
$21.88$\,dB and outscores all six open-source generators on both metrics while
...
\emph{viewpoint-dependent structure} beyond local texture, comparing EgoExo-Gen,
the global gate, and spatial routing against the ground truth at matched time
...
which are the parts of the frame a mask leaves unspecified. Spatial routing
follows the ground-truth framing of these regions more closely than the global
gate, which matches Eq.~\ref{eq:dilution} in that a shared scalar cannot raise
caption influence on the scene without also raising it where EgoExo-Gen is
already correct. Overall, the gains in Table~\ref{tab:main} correspond to visible scene
structure.
...
\vspace{-2mm}
\caption{Spatial routing preserves the viewpoint-dependent scene structure
that a mask leaves unspecified, including the layout of the cooking area in
...
frozen Qwen3-VL-32B on 13 exocentric frames, projects the 5120-dimensional
features into the video model, and injects them through a globally gated
cross-attention branch. It sits on the same frozen EgoExo-Gen base and differs
from the text branch only in what it carries.

As shown in Table~\ref{tab:main}, the continuous branch improves the
frame-level metrics yet reaches $44.5$ FVD, worse than the $42.6$ of EgoExo-Gen
itself, so it buys frame-level agreement without improving the video
distribution. The text branch under the same gate reaches $37.8$ FVD, raises
SSIM by $0.018$, and lowers LPIPS by $0.021$ against the continuous branch,
which places the text advantage before any routing is introduced. Two properties of a caption explain
...
As shown in Table~\ref{tab:main}, HOI-conditioned routing lowers FVD from $37.8$
to $33.3$ and LPIPS from $0.163$ to $0.144$, improving every metric without a
single inversion. Two features of the comparison support the dilution account. First, the route helps more on the
...
lowers SSIM from $0.660$ to $0.632$ and raises LPIPS to $0.166$, with the original
conditions winning on $88.7\%$ of clips, and randomizing the mask lowers SSIM to
$0.423$ and raises LPIPS to $0.504$, winning on every clip. Both anchors are
...
\caption{The trained model reads the content of both anchors, and the division
of labor is asymmetric, since randomizing the mask costs far more than
randomizing the caption while both degrade the output. Win is the fraction of clips on
which the original conditions give higher SSIM than the randomized input. SSIM
and Win are higher is better and LPIPS is lower is better. Best values are in
\textbf{bold}.}
\label{tab:ablation}
\begin{tabular}{lC{0.84cm}C{0.84cm}C{0.84cm}C{0.96cm}}
\toprule
\textbf{Condition} & \textbf{SSIM} & \textbf{PSNR} & \textbf{LPIPS} & \textbf{Win} \\
\midrule
\textbf{Original} & \textbf{0.660} & \textbf{22.28} & \textbf{0.144} & \\
$\;$cap.\ rand. & 0.632 & 21.59 & 0.166 & 88.7\% \\
$\;$mask rand. & 0.423 & 16.38 & 0.504 & 100.0\% \\
\bottomrule
\end{tabular}
\end{minipage}\hfill
...
retain the given frame. $\Delta$Corr is the residual correlation and
$\Delta$PSNR the residual fidelity, both higher is better, and the magnitude
ratio Mag.\ has target $1$. Best values are in \textbf{bold}.}
\label{tab:residual}
\begin{tabular}{lC{1.03cm}C{1.03cm}C{0.7cm}}
\toprule
\textbf{Method} & \textbf{$\Delta$Corr} & \textbf{$\Delta$PSNR} & \textbf{Mag.} \\
\midrule
\textit{Ground truth} & \textit{1.000} & & \textit{1.000} \\
\midrule
Stable Video Diff & 0.108 & 14.85 & 1.77 \\
Wan2.2-TI2V & 0.221 & 18.45 & 0.73 \\
LTX-Video & 0.233 & 18.86 & 0.61 \\
CogVideoX-I2V & 0.198 & 18.76 & 0.54 \\
Wan2.1-VACE & 0.003 & 18.09 & 0.71 \\
Wan2.1-Fun & 0.319 & 10.35 & 5.28 \\
\midrule
EgoExo-Gen & 0.561 & 19.80 & 1.18 \\
$\;+$ caption, global & 0.614 & 20.47 & 1.14 \\
$\;+$ caption, route & \textbf{0.647} & \textbf{21.17} & \textbf{1.04} \\
\bottomrule
\end{tabular}
\end{minipage}
\vspace{-4mm}
...
therefore subtract each video's own first frame and compare the residual fields
with the Pearson correlation ($\Delta$Corr), the residual PSNR ($\Delta$PSNR), and
the ratio of mean absolute residual magnitudes. A model holding its first frame
scores zero correlation by construction.

As shown in Table~\ref{tab:residual}, EgoExo-Gen and the two Any2Ego variants
reach $\Delta$Corr of $0.561$, $0.614$, and $0.647$, while the prompt-conditioned
baselines stay near $0.20$ and reproduce only $54\%$ to $73\%$ of the true residual
magnitude, so their frame-level scores come mostly from retained appearance.
Spatial routing brings the magnitude ratio to $1.04$ against $1.18$ for
EgoExo-Gen, so the textual anchor corrects the amount of motion as well as its
location. Across terciles of
ground-truth motion $\Delta$Corr rises from $0.565$ to $0.713$ for spatial
routing as real motion grows, while it stays flat near $0.22$ for the baselines,
which is the signature of a model copying its input frame. Overall, the residual analysis shows
...
All methods are evaluated on the same fixed split of $N{=}1{,}000$ clips
with 13 frames at $480\times480$, 50 denoising steps, and a fixed seed, where
CogVideoX uses its native 30 steps and we retain the first 13 of its 49 output
frames without temporal resampling. Per-clip PSNR, SSIM, and LPIPS are
...
$0.657$ across EgoExo-Gen, the global gate, and spatial routing, and median PSNR
moves from $21.27$ to
...
together across EgoExo-Gen, the global gate, and spatial routing under a shared
kernel bandwidth.}
\label{fig:dist}
\vspace{-4mm}
\end{figure}
