...
shown in Table~\ref{tab:main}, EgoExo-Gen already separates itself from every
open-source generator on FVD, and the textual anchor improves it further under
both injection rules, with spatial routing reaching $33.3$ FVD and leading on all
four metrics without a single inversion. More importantly, the freeze-frame row
...
continuous branch that extracts visual and instruction tokens from a frozen
Qwen3-VL-32B on the exocentric frames, projects them into the video model, and
injects them through a globally gated cross-attention branch. It sits on the same
frozen EgoExo-Gen base and differs from the text branch only in what it carries.

As shown in Table~\ref{tab:main}, the continuous branch improves the frame-level
metrics yet leaves FVD no better than EgoExo-Gen itself, so it buys frame-level
agreement without improving the video distribution. Under the same gate the text
branch improves both, which places the text advantage before any routing is
introduced. Two properties of a caption explain
...
As shown in Table~\ref{tab:main}, HOI-conditioned routing improves every metric
over the global gate without a single inversion, and the margin is largest on
FVD. Two features of the comparison support the dilution account. First, the route helps more on the
...
give higher SSIM than the randomized input.}
\label{tab:ablation}
...
\midrule
EgoExo-Gen (reproduced) & 0.561 & 19.80 & 1.18 \\
$\;\;+$ textual anchor, global gate & 0.614 & 20.47 & 1.14 \\
$\;\;+$ textual anchor, spatial routing & \textbf{0.647} & \textbf{21.17} & \textbf{1.04} \\
\bottomrule
\end{tabular}
\end{table}
...
raise residual correlation in order, reaching $0.647$, while the
